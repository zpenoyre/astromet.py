...
# rp should be in AU and Mtotal in Msun, unless G specified
# note that seems unreliable for phi=pi (but fine for infinitesimally below)
def phi_to_t(phi,rp,e,Mtotal,tPeri=0,G=Galt):
    cphi=np.cos(phi)
    sphi=np.sin(phi)
    if e==1:
        tau=np.sqrt((rp*(1+e))**3 /(G*Mtotal))
    ...
    if e>1:
        ...
        tau=np.sqrt(np.abs(rp/(1-e))**3 /(G*Mtotal))
        return tau*(e*shzeta-zeta)+tPeri

# inverse of above function
def t_to_phi(t,rp,e,Mtotal,tPeri=0,G=Galt):
    if e==1:
        tau=np.sqrt((rp*(1+e))**3 /(G*Mtotal))
        phi=findPhisParabolic(t,tau,tPeri=tPeri)
    if e<1:
        tau=2*np.pi*np.sqrt((rp/(1-e))**3 /(G*Mtotal))
    ...
    if e>1:
        tau=2*np.pi*np.sqrt(np.abs(rp/(1-e))**3 /(G*Mtotal))
